...
<body class="user-select">
	<section class="container-fluid">
		...
		<div
			...
			<h1 class="page-header">特产评价管理</h1>
					...
					<tbody id="load">
						<c:forEach items="${pageInfo.list}" var="item" varStatus="id">
							<tr>
					            ...
					            <a
									href="${pageContext.request.contextPath}/backComment/rejectSpecialtyComment?id=${item.id}"><button
											type="button" class="btn btn-warning">撤回</button></a> <a
									href="${pageContext.request.contextPath}/backComment/passSpecialtyComment?id=${item.id}"><button
			...
			<footer class="message_footer">
				<nav>
					<ul class="pagination pagenav"  
						...
						<li><a
							href="${pageContext.request.contextPath}/backComment/getSpecialtyComment?page=${pageInfo.prePage}"
						...
						<c:forEach var="i" begin="1" end="${pageInfo.pages}"
							...
							<c:choose>
								<c:when test="${pageUtils.currentPageNum==i}">
									<li class="active"><a
										href="${pageContext.request.contextPath}/backComment/getSpecialtyComment?page=${status.index}">${status.index}</a></li>
								</c:when>
								<c:otherwise>
									<li><a
										href="${pageContext.request.contextPath}/backComment/getSpecialtyComment?page=${status.index}">${status.index}</a></li>
						...
						<li><a
							href="${pageContext.request.contextPath}/backComment/getSpecialtyComment?page=${pageInfo.nextPage}"
